...
Isso importa principalmente pro RQ07 (bônus): ele cruza os resultados de RQ02/03/04 com a linguagem de cada repo (RQ05).

O `sprint_s01/all_rqs.csv` é esse CSV unificado: sai do `src/collection/collect_all_rqs.py` e traz as seis RQs por repositório. É a única fonte de dados dos 100 (ou 1000, no Lab01S02) repositórios — todo script de análise (`analyze_rq03.py`, `analyze_rq07.py`, `extra_idade_vs_popularidade.py`) lê dele por padrão. Os CSVs por dupla de RQ que cada integrante gerava em `sprint_s01/` durante a validação inicial foram removidos depois da unificação: `collect_sample_rq0X_rq0Y.py` continua existindo só para a validação rápida (10 repositórios) de cada integrante, salva em `amostra/`.
